...
class SupabaseInitAndSyncTest(unittest.TestCase):
    ...
    def test_maintain_workflow_defaults_fetch_days_to_nine(self):
        ...
        self.assertIn('ARGS=(--fetch-days "$FETCH_DAYS" --retention-days "$SUPABASE_RETENTION_DAYS")', text)
        self.assertEqual(workflow['jobs']['maintain_supabase']['env']['SUPABASE_RETENTION_DAYS'], '400')
