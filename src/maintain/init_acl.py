...
def main() -> None:
    ...
    if raw_path:
        ...
        token = f"acl-anthology-{int(args.year_end) - int(args.year_count) + 1}-{int(args.year_end)}"
    ...
    if not args.skip_fetch:
        ...
        run_step("Step 1 - fetch ACL Anthology", fetch_cmd)
    else:
    ...
    run_step("Step 2 - sync ACL papers to Supabase", sync_cmd)
