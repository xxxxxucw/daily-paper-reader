...
def build_biorxiv_paper_id(doi: str, version: str | int | None) -> str:
    ...
    return f"biorxiv-{safe_doi}-{version_text}"


def _build_biorxiv_abs_url(doi: str, version: str | int | None) -> str:
    version_text = _norm(version)
    if version_text and version_text.isdigit():
        version_text = f"v{version_text}"
    elif version_text and not version_text.startswith("v"):
        ...
        version_text = ""
    return f"https://www.biorxiv.org/content/{doi}{version_text}"


def _build_biorxiv_pdf_url(doi: str, version: str | int | None) -> str:
    return f"{_build_biorxiv_abs_url(doi, version)}.full.pdf"
...
def normalize_biorxiv_record(raw: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    return {
        "id": build_biorxiv_paper_id(doi, version),
        "source": "biorxiv",
        "source_paper_id": doi,
        "doi": doi,
        "version": version,
        ...
        "updated_at": datetime.now(timezone.utc).isoformat(),
        "link": _build_biorxiv_pdf_url(doi, version),
        "pdf_url": _build_biorxiv_pdf_url(doi, version),
        "abs_url": _build_biorxiv_abs_url(doi, version),
    }
...
def fetch_window_records(start_dt: datetime, end_dt: datetime, *, timeout: int = 60) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        for raw in collection:
            normalized = normalize_biorxiv_record(raw)
            if not normalized:
                continue
...
def fetch_biorxiv_metadata(
    *,
    ...
    group_start("Step 1 - fetch bioRxiv")
    log(
        "🌍 [bioRxiv Ingest] Window: "
    ...
    log(f"✅ All Done. Total unique bioRxiv papers fetched: {total_count}")
    ...
    if not output_file:
        ...
        output_file = os.path.join(raw_dir, f"biorxiv_papers_{run_token}.json")
    ...
    if total_count <= 0:
        log("⚠️ No bioRxiv papers found. Check your date range or network.")
    ...
    if windows and failed_windows == len(windows):
        raise SystemExit(
            f"bioRxiv 全部 {len(windows)} 个抓取窗口均失败，判定为上游不可用，本次维护中止。"
        )
