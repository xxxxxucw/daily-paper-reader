...
def main() -> None:
    ...
    if raw_path:
        ...
        raw_path = os.path.join(project_root, "archive", date_str, "raw", f"chemrxiv_papers_{date_str}.json")

    if not args.skip_fetch:
        fetch_cmd = [
            python,
            os.path.join(SCRIPT_DIR, "fetchers", "fetch_chemrxiv.py"),
            "--days",
    ...
    print(f"[INFO] ChemRxiv fetch 预检结果：count={fetch_count}，raw_path={raw_path}", flush=True)
    if fetch_count <= 0:
        print("[INFO] 本次 ChemRxiv 抓取无新增论文，已跳过 Supabase 同步。", flush=True)
        return
    ...
    run_step("Step 2 - sync ChemRxiv to Supabase", sync_cmd)
