...
def main() -> None:
    parser = argparse.ArgumentParser(description="抓取近 N 天 medRxiv 并同步到 Supabase（同 project 可用）。")
    ...
    if raw_path:
        ...
        raw_path = os.path.join(project_root, "archive", date_str, "raw", f"medrxiv_papers_{date_str}.json")

    if not args.skip_fetch:
        fetch_cmd = [
            python,
            os.path.join(SCRIPT_DIR, "fetchers", "fetch_medrxiv.py"),
            "--days",
        ...
        run_step("Step 1 - fetch medRxiv", fetch_cmd)
    else:
    ...
    run_step("Step 2 - sync medRxiv to Supabase", sync_cmd)
