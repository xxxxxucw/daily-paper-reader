...
def main() -> None:
    parser = argparse.ArgumentParser(description="抓取近三年 EMNLP Anthology 论文并同步到 Supabase。")
    ...
    if raw_path:
        ...
        token = f"emnlp-anthology-{int(args.year_end) - int(args.year_count) + 1}-{int(args.year_end)}"
        raw_path = os.path.join(project_root, "archive", date_str, "raw", f"{token}.json")

    if not args.skip_fetch:
        fetch_cmd = [
            python,
            os.path.join(SCRIPT_DIR, "fetchers", "fetch_acl_anthology.py"),
            "--conference",
            "EMNLP",
            "--year-end",
        ...
        run_step("Step 1 - fetch EMNLP Anthology", fetch_cmd)
    else:
    ...
    run_step("Step 2 - sync EMNLP papers to Supabase", sync_cmd)
