...
def main() -> None:
    ...
    raw_path = str(args.raw_input or "").strip() or default_raw_path("medrxiv_papers", run_date)
    ...
    init_cmd = [
        sys.executable,
        os.path.join(os.path.dirname(__file__), "init_medrxiv.py"),
        "--days",
    ...
    run_step("Maintain medRxiv", init_cmd)
    # 清理必须在同步成功之后执行：run_step 失败会抛异常，
    # 从而避免「抓取失败/零结果时只删不补」把表逐日削空。
    cleanup_backend(backend_key="medrxiv", retention_days=args.retention_days, skip_cleanup=args.skip_cleanup)
